  ...
  try {
    ...
    if (userAddress) {
      try {
        ...
        if (settings?.interests) {
          userInterests = settings.interests.split(",").map((s: string) => s.trim().toLowerCase());
        }
      } catch (err) {
    ...
    if (category && category !== "all" && category !== "recommended") {
      if (category.toLowerCase() === "trending") {
        queryBuilder = queryBuilder.eq("status", "trending");
      } else {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: newAudio, error } = await supabaseServer
      .from("Audio")
      .insert(
        withTimestamps({
          title,
